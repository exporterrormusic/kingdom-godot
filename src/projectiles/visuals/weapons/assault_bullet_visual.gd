...
class_name AssaultBulletVisual

@export var preview_radius: float = 7.2
@export var preview_color: Color = Color(1.0, 0.82, 0.42, 1.0)

const SMG_GLOW_SHADER := preload("res://src/projectiles/shaders/assault_normal_glow.gdshader")
...
func _ready() -> void:
	if bullet_sprite:
		...
		shader_material.set_shader_parameter("glow_intensity", 0.3)
...
func update_visual(direction: Vector2, radius: float, color: Color, _context: Dictionary = {}) -> void:
	...
	if bullet_sprite:
		...
		var glow_intensity: float = 0.1 if is_day_scene else 0.3
		var glow_size: float = 0.001 if is_day_scene else 0.004
